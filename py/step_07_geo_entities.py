...
def build() -> list[str]:
    ...
    parts.append(vu.svg_hash_node(*land, r, "Land", "land_{hash}", fill=GEO["fill"], stroke=GEO["stroke"]))
    parts.append(vu.svg_hash_node(*bundesland, r, "Bundesland", "bundesland_{hash}", fill=GEO["fill"], stroke=GEO["stroke"]))
    parts.append(vu.svg_hash_node(*kreis, r, "Kreis", "kreis_{hash}", fill=GEO["fill"], stroke=GEO["stroke"]))
    parts.append(vu.svg_hash_node(*gemeinde, r, "Gemeinde", "gemeinde_{hash}", fill=GEO["fill"], stroke=GEO["stroke"]))
    parts.append(vu.svg_box(fx, fy, fw, fh, "Fundstelle", "crm:E27_Site", fill=SITE["fill"], stroke=SITE["stroke"],
                             stroke_width=2.0))

    parts.append(vu.svg_arrow_labeled(fx, cy, gemeinde[0] + r, cy, "inGemeinde"))
    parts.append(vu.svg_arrow_labeled(gemeinde[0] - r, cy, kreis[0] + r, cy, "inKreis"))
    parts.append(vu.svg_arrow_labeled(kreis[0] - r, cy, bundesland[0] + r, cy, "inBundesland"))
    parts.append(vu.svg_arrow_labeled(bundesland[0] - r, cy, land[0] + r, cy, "inLand"))
    ...
    parts.append(f'<text x="60" y="272" font-family="Fira Sans" font-size="14.5" fill="{vu.TEXT_DARK}">'
                 f'Each of the four levels can additionally carry up to five external identifiers:</text>')
    ...
    parts.append(vu.svg_dashed_container(rx, ry_, rw, rh,
                                          "Georeferenzierung carries the location's certainty grade (Tab. 5)"))
    rows = [
        ("genauigkeit_m = 0", "fslwb:Q23", "high", "direct database take-over"),
        ("50\u2013500 m", "fslwb:Q15", "medium", "assignable to a Flur/Gemeindeteil"),
        ("800\u20135000 m", "fslwb:Q24", "low", "Gemeinde level only (centroid)"),
        ("x = y = 0", "fslwb:Q113", "dubious", "coordinate error, no sf:Point"),
    ]
    ty = ry_ + 74
    ...
    return vu.write_figure(OUT, "geo-entities", "\n".join(parts), zoom=1.5)
