...
def build(lang: str = "en") -> list[str]:
    ...
    parts.append(vu.svg_dashed_container(bx - 20, 55, bw + 40, 660,
                                          tt("CSV-Anreicherung (\u00a73)", "CSV enrichment (\u00a73)")))
    sh = 78
    s1 = (bx, 140)
    s2 = (bx, 580)
    s3 = (bx, 360)
    ...
    parts.append(vu.svg_arrow(csv_in[0] + aw, csv_in[1] + ah / 2, s1[0], s1[1] + sh / 2))
    parts.append(vu.svg_arrow(ref_in[0] + aw, ref_in[1] + ah / 2, s2[0], s2[1] + sh / 2))
    parts.append(vu.svg_arrow(s1[0] + bw / 2, s1[1] + sh, s3[0] + bw / 2 - 30, s3[1]))
    parts.append(vu.svg_arrow(s2[0] + bw / 2, s2[1], s3[0] + bw / 2 + 30, s3[1] + sh))
    ...
    parts.append(vu.svg_dashed_container(dx - 20, 55, dw + 40, 660,
                                          tt("LOD-Transformation (\u00a76)", "LOD transformation (\u00a76)")))
    build_ = (dx, 300)
    shapes = (dx, 460)
    shacl_val = (dx, 620)
    ...
    parts.append(vu.svg_arrow(build_[0] + dw, build_[1] + sh / 2 - 6, ex, data_ttl[1] + eh - 10))
    parts.append(vu.svg_arrow(build_[0] + dw, build_[1] + sh / 2 + 6, ex, bundle_ttl[1] + eh / 2))
    parts.append(vu.svg_arrow(build_[0] + dw, build_[1] + sh / 2 + 20, ex, prov_lod[1] + 10,
                               dashed=True, stroke=P["prov"]["stroke"]))
    parts.append(vu.svg_arrow(shacl_val[0] + dw, shacl_val[1] + sh / 2, ex, shacl_rep[1] + eh / 2))
    ...
    parts.append(vu.svg_arrow(data_ttl[0] + ew, data_ttl[1] + eh / 2, fx, val[1] + 15))
