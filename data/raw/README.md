...
| `bb5kbc-csv-issues.md` | 06 (cross-checked, revision 2026-09-11) | item 6 (the real, but already-fixed, German/whitespace heterogeneity in `dating_certainty_start`/`_end`) -- see `data/fst_wgs84_lit_enriched.csv` below for the correction that made this citation necessary |
| `data/fst_wgs84_lit_enriched.csv` | 05, 06, 08, 09 (verification only, revision 2026-09-11) | **not previously opened** -- the first versions of these four figures used invented example FIDs and free-text values while claiming in their docstrings to be real. Fixed by actually querying this file (`csv.DictReader` + `Counter()`) for every FID, dating value and `dating_certainty_range` string now shown; see `PRIMER.md` S31 for the full correction and the rule adopted afterwards (open the CSV before writing any "real value" claim, or say plainly that a value wasn't checked against it) |
