...
Every figure is a single self-contained 7:4 diagram (1750x1000 px), generated
as versioned SVG and rasterised to PNG in-process via [resvg-py][resvg], with
the vendored Fira Sans -- no Mermaid CLI, no Node, no system fonts required.
No title header or source-citation footer is baked in: these are
general-purpose diagram assets meant to be dropped into a slide, a paper
figure or a poster, each of which supplies its own caption. Same house
pattern as the sibling repo [crossy-visuals][crossy].
...
| # | Title | What it shows |
|---|---|---|
| 00 | Fundstelle hub | The mental model: a Fundstelle at the centre of seven relationships |
...
| 09 | Kulturen | How Kulturgruppe itself is modelled (`crm:E4_Period`, dedup, external ID) |

## Running it

```bash
pip install -r requirements.txt
python main.py                # all figures, into img/<NN-topic>/*.svg + *.png
python main.py --list         # print the step table
python main.py --only 04      # one figure
...
img/NN-topic/              generated SVG + PNG (versioned; see PRIMER.md A3)
```
